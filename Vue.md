...
v-bind指令只能实现数据的单项绑定，通过**v-model**可以做到**双向绑定**，即在页面中输入内容(表单时)Vue实例中的**属性也会一起变化**，但是注意v-model只能应用在**表单类元素上(输入类元素)**



v-model:value可以简写为**v-model**
...
<img src="assets/image-20241027095557126-1729994679241-29.png" alt="image-20241027095557126" style="zoom:50%;" /><img src="assets/image-20241027095543924.png" alt="image-20241027095543924" style="zoom:50%;" />
...
展示在页面上的数据用**计算属性**来做，它是由源数组**按照关键词过滤后**得到的结果
...
脚手架的配置属性都是**隐藏**起来的，想要**查看**配置文件的内容需要使用**vue inspect >output.js**
...
一个组件想要**复用**，数据就**不能写死**，应该要做到在调用时能**传输数据**

![image-20241104110540649](assets/image-20241104110540649.png)



在组件实例对象中配置**props**属性，值可以是一个数组里面存放外部传入的属性，没有传入的默认是undefined，这些属性最终会**挂载到实例对象**上，因此在组件对象中要通过**this**使用，外部(**模板**中)可以直接操作。

![image-20241104112236437](assets/image-20241104112236437.png)



传输的属性默认是字符串类型，如果想要用数值类型，最好的方法是在调用方传数据时**用v-bind语法**，将值绑定为一个**表达式**

通过**数据绑定**还可以传入**对象，方法**等

![image-20241104112621368](assets/image-20241104112621368.png)



用对象指定接受的参数的**类型**，防止调用者传入错误类型的参数
...
提取出组件中**共同的配置**放到一个JS文件中，该文件与components文件夹同级
...
组件中样式可能会重叠，因此需要在style标签中加上**scoped**关键字，来保证组件中的样式只会影响到这个组件本身
